document.addEventListener("DOMContentLoaded", function () {
  ...
  function editStudent(studentId) {
    ...
    if (student) {
      ...
      document.getElementById("edit-student-edu").value =
  ...
  function addNewStaff() {
    ...
    const newId =
      appData.staff.length > 0
        ? Math.max(...appData.staff.map((s) => s.id)) + 1
        : 1;
  ...
  function viewStaffDetails(staffId) {
    ...
    if (staff) {
      ...
      detailsContent.innerHTML = `
        ...
        <div class="detail-row">
          <strong>Status:</strong><span class="status-available">${
            staff.status
          }</span>
        </div>
      `;
  ...
  function deleteStaff(staffId) {
    ...
    if (staffIndex !== -1) {
      ...
      addActivity(
        "staff",
        `Staff member ${staff.name} (ID: ${staffId}) was deleted`
      );
